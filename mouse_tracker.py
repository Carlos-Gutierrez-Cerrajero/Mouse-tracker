...
def dict_writer(d, key_length, file):
    for key in d:
        file.write(
        str(key).ljust(key_length)
        + str(d[key][0]) + ", "
...
try:
    while True:                                                                  #Runs code within an infinite loop for sustained execution
        ...
        if keyboard.is_pressed("alt") and keyboard.is_pressed("l"):
            ...
            log = open("mouse_tracker_log.txt", "w", encoding ="utf-8")         #Creates or overwrites mouse_tracker_log.txt
            
            if len(custom_log) > 1:
                log.write("Log of custom tags:\n\n")                            #Writes the title for the first dictionary and spaces it
                dict_writer(custom_log, 50, log)                                #Loops through every dictionary item and writes it
            ...
            if len(cursor_log) > 1:
                ...
                dict_writer(cursor_log, 5, log)                                 #Loops through every dictionary item and writes it
